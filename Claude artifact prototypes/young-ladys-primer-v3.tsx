            ...
            {currentContent.choices && (
              <div className="space-y-3 mt-6">
                ...
                {currentContent.choices.map((choice, index) => {
                  const isImplemented = stories[choice.action] !== undefined;
                  return (
                    <button
                      key={index}
                      onClick={() => isImplemented && handleChoice(choice.action)}
                      className={`w-full text-left p-4 rounded transition-all duration-300 
                               ...
                               ${isImplemented 
                                 ...
                                 : 'bg-gray-100/20 text-gray-500/70 border-gray-300/20 cursor-not-allowed'}`}
                      style={{
                        boxShadow: isImplemented ? '1px 1px 4px rgba(0,0,0,0.05)' : 'none',
                        fontFamily: 'Baskerville, serif'
                      }}
                      disabled={!isImplemented}
                    >
                      <span className={`font-medium relative z-10 ${!isImplemented && 'italic'}`} 
                            style={{ letterSpacing: '0.02em' }}>
                        {choice.text}
                        {!isImplemented && <span className="text-gray-400/70 text-sm ml-2">[coming soon]</span>}
                      </span>
                      {isImplemented && (
