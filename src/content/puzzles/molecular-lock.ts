export const molecularLockPuzzle = {
  molecular_lock: {
    ...
    content: ({ readerName }: { readerName: string }) => `${readerName} discovers a sealed vault within the Artificer Queen's workshop, its entrance protected by an intricate lock that shimmers with shifting molecular patterns.
...
Which molecular state holds the key to unlocking this chamber?`,
  ...
  chamber_one_ice: {
    ...
    content: ({ readerName }: { readerName: string }) => `Excellent reasoning, ${readerName}! The slow-moving molecules form ice - the most structured state of water. The chamber recognizes this principle of molecular order and clicks open with a crystalline chime.
...
<em>"Choose the pattern that gives the dragon's scales their legendary strength."</em>`,
  ...
  chamber_one_liquid: {
    ...
    content: ({ readerName }: { readerName: string }) => `The lock shimmers but doesn't open. ${readerName} realizes that while liquid water is important, this particular chamber seems to want the most ordered state of matter.`,
    choices: [
  ...
  chamber_one_steam: {
    ...
    content: ({ readerName }: { readerName: string }) => `The chamber glows briefly but the lock remains sealed. ${readerName} considers that perhaps this chamber values structure over chaos.`,
    choices: [
      { text: "Try Molecule A (slow vibration)", action: "chamber_one_ice" },
  ...
  lock_hint_one: {
    ...
    content: ({ readerName }: { readerName: string }) => `"Think about when water is most organized, ${readerName}," the Primer suggests. "The ancient builders valued order and structure. When do molecules arrange themselves in the most predictable, organized way?"`,
    choices: [
  ...
  chamber_two_graphene: {
    ...
    content: ({ readerName }: { readerName: string }) => `Brilliant deduction, ${readerName}! The hexagonal sheets represent graphene - carbon atoms arranged in perfect hexagons, creating material stronger than steel yet flexible like fabric. The second chamber opens with a harmonic resonance.
...
<em>"Which molecule would plants choose to capture energy from sunlight?"</em>`,
  ...
  chamber_two_polymer: {
    ...
    content: ({ readerName }: { readerName: string }) => `The chamber acknowledges the flexibility of polymer chains, but something stronger is needed for dragon scales. ${readerName} thinks about what could be even more robust.`,
    choices: [
  ...
  lock_hint_two: {
    ...
    content: ({ readerName }: { readerName: string }) => `"Dragon scales must be both strong and flexible, ${readerName}," the Primer explains. "Think about how carbon atoms can arrange themselves in flat sheets - strong like diamond, but able to slide past each other like fabric."`,
    choices: [
  ...
  chamber_three_green: {
    ...
    content: ({ readerName }: { readerName: string }) => `Perfect! ${readerName} understands that plants appear green because chlorophyll absorbs red and blue light for energy, reflecting only green light back to our eyes. The molecular lock recognizes this wisdom about how molecules communicate with light.
...
<em>"You have demonstrated understanding of molecular behavior, structural engineering, and the language of light. These are the foundations upon which all nanotechnology is built. This is the knowledge Princess Elara used to master her transformation."</em>`,
  ...
  chamber_three_red: {
    title: "Not Quite",
    content: ({ readerName }: { readerName: string }) => `The red molecule only reflects red light - it's not capturing much energy for the plant. ${readerName} thinks about what colors plants actually use.`,
    choices: [
  ...
  lock_hint_three: {
    ...
    content: ({ readerName }: { readerName: string }) => `"Think about why plants appear green to your eyes, ${readerName}," the Primer suggests. "What they reflect is different from what they absorb. Plants are green because they use other colors for energy."`,
    choices: [
